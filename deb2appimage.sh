...
function getlatestdeb() {
    ...
    curl -sL "$LATEST_DEB_URL" -o "$HOME"/.cache/deb2appimage/debs/"$DEB_NAME".deb || echo "$DEB_NAME.deb" "URL: $LATEST_DEB_URL" && getlatestdeb $DEB_NAME $DEB_DISTRO $DEB_RELEASE $DEB_ARCH
}
