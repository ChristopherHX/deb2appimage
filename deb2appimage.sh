...
function getlatestdeb() {
    ...
    echo "Downloading $DEB_NAME.deb $LATEST_DEB_URL"
    curl -sL "$LATEST_DEB_URL" -o "$HOME"/.cache/deb2appimage/debs/"$DEB_NAME".deb || getlatestdeb $DEB_NAME $DEB_DISTRO $DEB_RELEASE $DEB_ARCH
}
